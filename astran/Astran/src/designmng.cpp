...
int DesignMng::getCommandCode(vector<string> shell_cmd)
{
    ...
    if (shell_cmd[0] == commands_lst[COMMENT].name)
        return COMMENT;
    ...
    for (int i = 0; i < NR_COMMANDS; ++i)
    {
        ...
        if (reg_cmd.size() == shell_cmd.size())
        {
            ...
            if (same_cmd)
            {
                ...
                for (j = 0; j < reg_cmd.size(); ++j)
                    if (((reg_cmd[j].substr(0, 4) == "<int") || (reg_cmd[j].substr(0, 6) == "<float")) &&
                        (!isNumber(shell_cmd[j])))
...
bool DesignMng::readCommand(string cmd)
{
    try
    {
        ...
        for (int c = 0; c < cmd.size(); ++c)
        {
            ...
            if ((c == cmd.size() - 1 && cmd[cmd.size() - 1] != ' ') ||
                (cmd[c] == ' ' && !insideaspas && fim - inicio >= 0))
                ok = true;
        ...
        if (words.size() != 0)
        {
            ...
            case EXPORT_LAYOUT:
            {
                ...
                if (circuit->getLayout(upcase(words[2])))
                {
                    if (tipo == "GDS")
                    {
                        ...
                        for (layers_it = circuit->getLayout(words[2])->layers.begin();
                             layers_it != circuit->getLayout(words[2])->layers.end(); layers_it++)
                        {
                            if (!layers_it->second.empty())
                            {
                                int layer = strToInt(rules->getGDSIIVal(layers_it->first));
                                for (layer_it = layers_it->second.begin(); layer_it != layers_it->second.end();
                                     layer_it++)
                                {
                                    ...
                                    if (x2 - x1 != 0 & y2 - y1 != 0)
                                    {
                                        ...
                                        g.generateBox(layer, 1.25*x1, 1.25*y1, 1.25*x2, 1.25*y2);
                                        //                                            cout << layer << "-"<< x1 << "-"
                                        //                                            << y1 << "-" << x2 << "-" << y2 <<
                                        //                                            endl;
                        ...
                        for (labels_it = circuit->getLayout(words[2])->labels.begin();
                             labels_it != circuit->getLayout(words[2])->labels.end(); labels_it++)
                        {
                            ...
                            g.generateLabel(strToInt(rules->getGDSIIVal(MET1)), 2.5 * labels_it->pt.getX(),
                                            2.5 * labels_it->pt.getY(), tmp);
                            g.generateEndelement();
                        }
            ...
            case PLACE_INSTANCE:
            {
                ...
                tmp->placeCell(upcase(words[3]), atoi(words[4].c_str()), atoi(words[5].c_str()), atoi(words[6].c_str()),
                               atoi(words[7].c_str()));
                break;
            }
            case PLACE_AUTOFLIP:
            ...
            case SET_MARGINS:
                circuit->setMargins(atof(words[2].c_str()), atof(words[3].c_str()), atof(words[4].c_str()),
                                    atof(words[5].c_str()));
                break;
            ...
            case CELLGEN_AUTOFLOW:
                cout << "-> Autoflow with trackRequired = " << words[2] << endl;
                autocell->autoFlow(lpSolverFile, atoi(words[2].c_str()));
                break;
            ...
            case CELLGEN_PLACE:
                autocell->placeTrans(true, atoi(words[2].c_str()), atoi(words[3].c_str()), atoi(words[4].c_str()),
                                     atoi(words[5].c_str()), atoi(words[6].c_str()), atoi(words[7].c_str()),
                                     atoi(words[8].c_str()));
                break;
            ...
            case CELLGEN_SETARCCOST:
                cout << autocell->getRouting()->setArcCost(atoi(words[2].c_str()), atoi(words[3].c_str()),
                                                           atoi(words[4].c_str()));
                break;
            ...
            case CELLGEN_COMPACT:
                if (!autocell->compact(lpSolverFile, atoi(words[2].c_str()), atoi(words[3].c_str()),
                                       atoi(words[4].c_str()), atoi(words[5].c_str()), atoi(words[6].c_str()),
                                       atoi(words[7].c_str()), atoi(words[8].c_str()), atoi(words[9].c_str()),
                                       atoi(words[10].c_str()), atoi(words[11].c_str())))
