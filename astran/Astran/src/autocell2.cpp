                       ...
                       int timeLimit)
{
    ...
    //zc Adding VCC and GND labels
    Pin p1;
    p1.setX(0);
    p1.setY(560);
    p1.setLayer(MET1P);
    currentLayout.setPin("VCC",p1);
    currentLayout.addLabel("VCC",p1);
    Pin p2;
    p2.setX(0);
    p2.setY(0);
    p2.setLayer(MET1P);
    currentLayout.setPin("GND",p2);
    currentLayout.addLabel("GND",p2);
    ...
    //zc writing to file:
    string cellsizefile = "../build/bin/CellSizes/" + currentLayout.getName() + "_cellsize.txt";
    ofstream outputFile(cellsizefile, ios::app);
    if (outputFile.is_open()) {
        outputFile << currentLayout.getName() << " " << currentCircuit->getRowHeight() << " " << currentCircuit->getnWellPos() << "      ";
        float Area=static_cast<float>(currentLayout.getWidth()) / currentRules->getScale() * static_cast<float>(currentLayout.getHeight()) / currentRules->getScale();
        outputFile << static_cast<float>(currentLayout.getWidth()) / currentRules->getScale() << " x " << static_cast<float>(currentLayout.getHeight()) / currentRules->getScale() << "       " << Area << endl;
        outputFile << endl;

        outputFile.close();
        cout << "Data written to ../build/bin/CellSizes.txt successfully." << endl;
    }
    else{
