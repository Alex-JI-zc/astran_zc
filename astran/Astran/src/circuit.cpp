...
Circuit::Circuit()
{
    ...
    nWellBorder = 0.235f;
    pnSelBorder = 0.09f;
    nWellPos = 0.70f;//zc 1.14f 1.5
    cellTemplate = "Tapless";
    vddNet = "VCC";
    gndNet = "GND";
    cellsHeight = 7;//zc 13 16
...
bool Circuit::setMargins(float l, float r, float t, float b)
{
    ...
    cout << "-> Margins: L=" << c2lMargin * getHPitch() << "um  R=" << c2rMargin * getHPitch()
         << "um  T=" << c2tMargin * getVPitch() << "um  B=" << c2bMargin * getVPitch() << "um" << endl;
    return true;
}
...
/*
 ...
 for(map<string,CellInstance>::iterator inst_it=cell->getInstances().begin(); inst_it!=cell->getInstances().end();
 ++inst_it){ list<string> inouts; for(int c=0; c<inst_it->second.ports.size(); ++c)
...
CellNetlst Circuit::getFlattenCell(string name)
{
    ...
    else
    {
        ...
        for (map<string, CellInstance>::iterator instance_it = instances.begin(); instance_it != instances.end();
             ++instance_it)
        {
            ...
            {
                cout << "-> ERROR: Call for cell " << instance_it->second.subCircuit
                     << " have different number of IOs in subcircuit " << name << endl;
                return tmp;
            }
            ...
            for (n = 0; n < currentCell.size(); ++n)
            {
                ...
                tmp.insertTrans(currentTrans.name, currentCell.getNetName(currentTrans.drain),
                                currentCell.getNetName(currentTrans.gate), currentCell.getNetName(currentTrans.source),
                                currentTrans.type, currentTrans.length, currentTrans.width);
            }
        }
    }
    return tmp;
...
void Circuit::calculateCellsPins()
{
    ...
    for (map<string, CellNetlst>::iterator cellNetlsts_it = cellNetlsts.begin(); cellNetlsts_it != cellNetlsts.end();
         ++cellNetlsts_it)
    {
        ...
        if (layouts_it != layouts.end())
        {
            ...
            for (int x = hGrid / 2; x <= layouts_it->second.getWidth(); x += hGrid)
            {
                for (int y = vGrid; y <= layouts_it->second.getHeight() - vGrid; y += vGrid)
                {
                    ...
                    bool fail = false;
                    int finalX, finalY;
                    for (map<layer_name, list<Box>>::iterator layers_it = layouts_it->second.layers.begin();
                         layers_it != layouts_it->second.layers.end(); layers_it++)
                    {
                        if (layers_it->first == MET1)
                        {
                            for (list<Box>::iterator layer_it = layers_it->second.begin();
                                 layer_it != layers_it->second.end(); layer_it++)
                            {
                                int dx =
                                    max(layer_it->getX1(), x - halfPinSize) - min(layer_it->getX2(), x + halfPinSize);
                                int dy =
                                    max(layer_it->getY1(), y - halfPinSize) - min(layer_it->getY2(), y + halfPinSize);
                                if (dx <= -(2 * halfPinSize - 1) && dy <= -(2 * halfPinSize - 1))
                                {
                                    ...
                                    if (connectedNet != layer_it->getNet() ||
                                        (aroundNet != "none" && aroundNet != connectedNet))
                                        fail = true;
                                }
                                else if ((dx < 0 && dy < 0) ||
                                         ...
                                         (dx >= 0 && dy >= 0 &&
                                          sqrt(static_cast<float>(dx * dx + dy * dy)) <
                                              currentRules->getRule(S1M1M1) - 1))
                                {
                                    if (aroundNet == "none")
                                        aroundNet = layer_it->getNet();
                                    if (aroundNet != layer_it->getNet() ||
                                        (connectedNet != "none" && aroundNet != connectedNet))
                                        fail = true;
                                }
                            }
                        }
